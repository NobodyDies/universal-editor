...
| settings[handlers] | object | Объект с описанием слушателей компонента. | - | - |
| settings[change] | function | Слушатель, который вызывается при изменении значения компонента. Функция в качестве аргументов принимает новое и старое значение. | - | - |
...
## Отслеживание значения компонента

При конфигурировании компонента очень часто нужен слушатель его значения, чтобы реагировать на изменение. Параметр `handlers.change` позволяет подключить его через логику редактора. Это избавит от создания дополнительного вотчера. Функция имеет два аргумента `newValue` и `oldValue`, которые являются новым и старым значением компонента, соответственно.

Например, имеется компонент автозаполнения (`ue-autocomplete`) и таблица значений (`ue-grid`). В зависимости от выбранного значения нужно отправлять запрос с фильтром и обновлять данные таблицы. 
...
```javascript
{
    ...
    settings: {
        ...
        handlers: {
            change: function(newValue, oldValue) {
               updateTable(newValue); // обновляем данные таблицы
            }
        }
    }
}
