...
| settings[change] | function | Слушатель изменения значения компонента. | - | - |
...
При конфигурировании компонента очень часто нужен слушатель его значения, чтобы реагировать на изменение. Параметр `change` позволяет подключить его через логику редактора. Это избавит от создания дополнительного вотчера. Функция имеет два аргумента `newValue` и `oldValue`, которые являются новым значение и старым значением компонента, соответственно.
